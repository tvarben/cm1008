...
typedef struct {
  ...
  int nrOfEnemies_1;
  int nrOfEnemies2; // counter for new nr of new enemies
...
int initiate(Game *pGame) {
  ...
  pGame->pStartImage =
      SDL_CreateTextureFromSurface(pGame->pRenderer, tempSurface);
  ...
  pGame->pStartImage2 =
      SDL_CreateTextureFromSurface(pGame->pRenderer, tempSurface2);
  ...
  pGame->pExitText =
      createText(pGame->pRenderer, 238, 168, 65, pGame->pSmallFont, "Exit",
                 WINDOW_WIDTH / 2, 570);
  pGame->pPauseText = createText(pGame->pRenderer, 238, 168, 65, pGame->pFont,
                                 "PAUSED", WINDOW_WIDTH / 2, WINDOW_HEIGHT / 4);
  pGame->pMenuText = createText(pGame->pRenderer, 238, 168, 65, pGame->pFont,
                                "MENU", WINDOW_WIDTH / 2, 450);
  pGame->pGameOverText =
...
void run(Game *pGame) {
  ...
  while (isRunning) {
    ...
    const SDL_Rect *startRect = getTextRect(
        pGame->pSingleplayerText); // Hämta position för rect för Start-texten
    const SDL_Rect *exitRect = getTextRect(
    ...
    if (SDL_PointInRect(&mousePoint, startRect)) {
      ...
      setTextColor(pGame->pSingleplayerText, 238, 168, 65, pGame->pSmallFont,
                   "Singleplayer");
    }
    ...
    if (SDL_PointInRect(&mousePoint, multiRect)) {
      setTextColor(pGame->pMultiplayerText, 255, 100, 100, pGame->pSmallFont,
                   "Multiplayer");
    } else {
    ...
    while (SDL_PollEvent(&event)) {
      if (event.type == SDL_QUIT) {
        ...
        printf("CURRENT NUMBER OF ENEMIES: %d \n",
               ((pGame->nrOfEnemies_1) + (pGame->nrOfEnemies2)));
        ...
        stringIndex--;
                 ...
                 event.type == SDL_KEYDOWN) {
        SDL_Keycode keycode = event.key.keysym.sym;
        if ((char)keycode <= '9' && (char)keycode >= '0' && stringIndex < 15 ||
            (char)keycode == '.') {
          ipAdress[stringIndex] = (char)keycode;
          stringIndex++;
        }
    ...
    if (pGame->state == ONGOING) {
      ...
      SDL_SetRenderDrawColor(
          pGame->pRenderer, 0, 0, 0,
          0); // Important to set the color before clearing the screen
      SDL_RenderClear(
      ...
      Text *pKillCountText =
          createText(pGame->pRenderer, 238, 168, 65, pGame->pSmallFont,
                     scoreText, WINDOW_WIDTH / 2, WINDOW_HEIGHT / 3);
...
void spawnEnemies(Game *pGame, int ammount) {
  for (int i = 0; i < ammount; i++) {
    pGame->pEnemies[pGame->nrOfEnemies_1] =
        createEnemy(pGame->pEnemyImage, WINDOW_WIDTH, WINDOW_HEIGHT);
    pGame->pEnemies2[pGame->nrOfEnemies2] =
        createEnemy2(pGame->pEnemyImage2, WINDOW_WIDTH, WINDOW_HEIGHT);
    pGame->nrOfEnemies_1++;
    pGame->nrOfEnemies2++;
  }
}
...
bool areTheyAllDead(Game *pGame) {
  for (int i = 0; i < pGame->nrOfEnemies_1; i++) {
    if (isEnemyActive(pGame->pEnemies[i]) == true ||
        isEnemy2Active(pGame->pEnemies2[i]) == true) {
      return false;
    }
  }
  return true;
