...
Ship *createShip(int playerId, SDL_Renderer *renderer, int windowWidth,
  ...
  SDL_QueryTexture(pShip->texture, NULL, NULL, &pShip->shipRect.w,
                   &pShip->shipRect.h);
  pShip->shipRect.w /= 4;
  pShip->shipRect.h /= 4;
  pShip->xStart = pShip->x = pShip->shipRect.x =
      windowWidth / 2 - pShip->shipRect.h / 2;
...
void updateShipOnClients(Ship *pShip, int shipId, int myShipId) {
  const float lerpFactor =
      0.75f; // 75% Adjust this value to control the interpolation speed, the
  ...
  if (shipId == myShipId) {
    ...
    if (dx < -correctionFactor || dx > correctionFactor ||
        dy < -correctionFactor || dy > correctionFactor) {
...
void setShoot(Ship *pShip, bool value) { pShip->isShooting = value; }
