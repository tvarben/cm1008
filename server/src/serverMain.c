...
int initiate(Game *pGame) {
  ...
  pGame->pWindow =
      SDL_CreateWindow("Server", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
  ...
  pGame->pLobbyText =
      createText(pGame->pRenderer, 238, 168, 65, pGame->pFont,
...
void printPacketsData(Game *pGame) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    printf("  Ship %d: x = %.2f, y = %.2f, angle = %.2f, alive = %d\n", i,
           sd->ships[i].x, sd->ships[i].y, sd->ships[i].vx, sd->ships[i].vy);
  }
  // Print enemy count
  printf("  Enemies_1: %d active, %d to spawn\n", sd->nrOfEnemies_1,
         sd->nrOfEnemiesToSpawn_1);
  // Print each enemy_1's data
  for (int i = 0; i < sd->nrOfEnemies_1; i++) {
    printf("    Enemy_1[%d]: x = %.2f, y = %.2f, alive = %d\n", i,
           sd->enemies_1[i].x, sd->enemies_1[i].y, sd->enemies_1[i].active);
  }
