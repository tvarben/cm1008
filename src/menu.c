...
void showNetworkMenu(SDL_Renderer *renderer, TTF_Font *font,
  ...
  SDL_Surface *surface =
      TTF_RenderText_Solid(font, "             Enter IP Adress: ", textColor);
  SDL_Texture *texture = SDL_CreateTextureFromSurface(renderer, surface);
  SDL_Rect textRect = {x + (menuWidth - surface->w) / 2, y + 30, surface->w,
                       surface->h};
  ...
  if (strlen(ipAdress) > 0) {
    ...
    if (ipSurface) {
      SDL_Texture *ipTexture =
          SDL_CreateTextureFromSurface(renderer, ipSurface);
...
void showMapMenu(SDL_Renderer *renderer, TTF_Font *font) {
  ...
  SDL_Texture *texture = SDL_CreateTextureFromSurface(renderer, surface);
  SDL_Rect textRect = {x + (menuWidth - surface->w) / 2, y + 20, surface->w,
                       surface->h};
  SDL_RenderCopy(renderer, texture, NULL, &textRect);
  SDL_FreeSurface(surface);
  SDL_DestroyTexture(texture);
  ...
  SDL_Rect easyRect = {x + 210,              // far left
  ...
  texture = SDL_CreateTextureFromSurface(renderer, surface);
  SDL_Rect hardRect = {x + menuWidth - surface->w - 225, // far right
                       y + menuHeight - 100, surface->w, surface->h};
