...
typedef struct {
  ...
  Text *pSingleplayerText, *pGameName, *pExitText, *pPauseText, *pScoreText,
      *pMultiplayerText, *pGameOverText, *pMenuText;
  Stars *pStars;
...
int initiate(Game *pGame) {
  ...
  pGame->pSingleplayerText =
      createText(pGame->pRenderer, 238, 168, 65, pGame->pFont, "Singleplayer",
                 WINDOW_WIDTH / 2, 330);
  pGame->pMultiplayerText =
      createText(pGame->pRenderer, 238, 168, 65, pGame->pFont, "Multiplayer",
                 WINDOW_WIDTH / 2, 450);
...
void run(Game *pGame) {
  ...
  while (isRunning) {
    ...
    if (pGame->state == ONGOING) {
      ...
      if (SDL_PointInRect(&mousePoint, startRect)) {
        setTextColor(pGame->pSingleplayerText, 255, 255, 100, pGame->pFont,
                     "Singleplayer");
      } else {
        setTextColor(pGame->pSingleplayerText, 238, 168, 65, pGame->pFont,
                     "Singleplayer");
      }
      ...
      if (SDL_PointInRect(&mousePoint, multiRect)) {
        setTextColor(pGame->pMultiplayerText, 255, 100, 100, pGame->pFont,
                     "Multiplayer");
      } else {
        setTextColor(pGame->pMultiplayerText, 238, 168, 65, pGame->pFont,
                     "Multiplayer");
      }
      ...
      SDL_RenderClear(
          pGame->pRenderer); // Clear the first frame when the game
...
void updateNrOfEnemies(Game *pGame) {
  if (getTime(pGame) > pGame->timeForNextEnemy &&
      pGame->nrOfEnemies < MAX_ENEMIES) {
