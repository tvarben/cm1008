...
int initiate(Game *pGame) {
  ...
  pGame->pWindow =
      SDL_CreateWindow("Client", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
  ...
  pGame->pSinglePlayerText =
      createText(pGame->pRenderer, 255, 0, 0, pGame->pSmallFont, "Singleplayer",
                 WINDOW_WIDTH / 2, 330);
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    pGame->pShips[i] =
        createShip(i, pGame->pRenderer, WINDOW_WIDTH, WINDOW_HEIGHT);
    pGame->pCannons[i] =
        createCannon(pGame->pRenderer, WINDOW_WIDTH, WINDOW_HEIGHT);
  }
  ...
  pGame->pStartImage_1 =
      SDL_CreateTextureFromSurface(pGame->pRenderer, tempSurface_1);
  ...
  pGame->pStartImage_2 =
      SDL_CreateTextureFromSurface(pGame->pRenderer, tempSurface_2);
...
void renderStartWindow(Game *pGame) {
  ...
  SDL_Rect dstRect_1 = {
      125, 500, 100, 100}; // adjust position and size, placering av planet/måne
  SDL_RenderCopy(pGame->pRenderer, pGame->pStartImage_1, NULL, &dstRect_1);
  SDL_Rect dstRect_2 = {
      1000, 125, 50, 50}; // adjust position and size, placering av planet/måne
            ...
            if (connectToServer(pGame)) {
              ...
              while (pGame->state != ONGOING) {
                if (SDLNet_UDP_Recv(pGame->pSocket, pGame->pPacket) == 1) {
                  if (strncmp((char *)pGame->pPacket->data, "ONGOING", 7) ==
                      0) {
                    // set a countdown
                    showCountdown(pGame);
                    pGame->state = ONGOING;
...
void printMultiplayerMenu(Game *pGame, char *pEnteredIPAddress,
  ...
  SDL_Surface *textSurface =
      TTF_RenderText_Solid(pGame->pSmallFont, pEnteredIPAddress, color);
  ...
  if (textSurface) {
    ...
    SDL_Texture *textTexture =
        SDL_CreateTextureFromSurface(pGame->pRenderer, textSurface);
  ...
  SDL_Surface *promptSurface1 = TTF_RenderText_Solid(
      pGame->pSmallFont, "Type in server IP ADDRESS and press ENTER", color);
  if (promptSurface1) {
  ...
  SDL_Surface *promptSurface2 = TTF_RenderText_Solid(
      pGame->pSmallFont, "Press ESCAPE to go to the MAIN MENU", color);
  if (promptSurface2) {
...
void updateWithServerData(Game *pGame) {
  ...
  pGame->shipId =
      serverData.sDPlayerId; ////// test  Kontrollera varför vi gör detta
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (pGame->pShips[i])
      updateShipsWithServerData(pGame->pShips[i], &serverData.ships[i], i,
                                pGame->shipId);
  }
...
void handleInput(SDL_Event *pEvent, Game *pGame) {
  ClientData cData;
  cData.cDPlayerId =
      pGame->shipId; // cDPlayerId not really needed. Server finds out which
                     // klient it is based on IP-address
...
MainMenuChoice handleMainMenuOptions(Game *pGame) {
  ...
  if (SDL_PointInRect(&mousePoint, singleRect))
    setTextColor(pGame->pSinglePlayerText, 255, 255, 255, pGame->pSmallFont,
                 "Singleplayer");
  else
    setTextColor(pGame->pSinglePlayerText, 255, 0, 0, pGame->pSmallFont,
                 "Singleplayer");

  if (SDL_PointInRect(&mousePoint, multiRect))
    setTextColor(pGame->pMultiPlayerText, 255, 255, 255, pGame->pSmallFont,
                 "Multiplayer");
  else
    setTextColor(pGame->pMultiPlayerText, 255, 0, 0, pGame->pSmallFont,
                 "Multiplayer");
  ...
  if (mouseState & SDL_BUTTON(SDL_BUTTON_LEFT)) {
    if (SDL_PointInRect(&mousePoint, singleRect))
      return MAINMENU_SINGLEPLAYER;
    if (SDL_PointInRect(&mousePoint, multiRect))
      return MAINMENU_MULTIPLAYER;
    if (SDL_PointInRect(&mousePoint, exitRect))
      return MAINMENU_EXIT;
  }
...
void showCountdown(Game *pGame) {
  ...
  snprintf(joinMsg, sizeof(joinMsg),
           "%d players have joined. Starting game in...", MAX_PLAYERS);
  Text *pJoinText =
      createText(pGame->pRenderer, 255, 0, 0, pGame->pSmallFont, joinMsg,
  ...
  while (countdown > 0 && pGame->isRunning) {
    ...
    if (now - lastTick >= 1000) {
      ...
      pGame->pCountdownText =
          createText(pGame->pRenderer, 255, 255, 255, pGame->pFont, buffer,
...
void closeGame(Game *pGame) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++)
    if (pGame->pShips[i])
      destroyShip(pGame->pShips[i]);
  for (int i = 0; i < MAX_PLAYERS; i++)
    if (pGame->pCannons[i])
      destroyCannon(pGame->pCannons[i]);
  if (pGame->pRenderer)
  ...
  if (pGame->pGameName)
    destroyText(pGame->pGameName);
  if (pGame->pSinglePlayerText)
    destroyText(pGame->pSinglePlayerText);
  if (pGame->pMultiPlayerText)
    destroyText(pGame->pMultiPlayerText);
  if (pGame->pExitText)
    destroyText(pGame->pExitText);
  if (pGame->pWaitingText)
    destroyText(pGame->pWaitingText);
  if (pGame->pStars)
    destroyStars(pGame->pStars);
  if (pGame->pStartImage_1)
    SDL_DestroyTexture(pGame->pStartImage_1);
  if (pGame->pStartImage_2)
    SDL_DestroyTexture(pGame->pStartImage_2);

  if (pGame->pFont)
    TTF_CloseFont(pGame->pFont);
  if (pGame->pSmallFont)
    TTF_CloseFont(pGame->pSmallFont);
